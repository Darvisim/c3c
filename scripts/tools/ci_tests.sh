...
run_cli_tests() {
    ...
    if [ -n "$SKIP_NETWORK_TESTS" ]; then
        ...
        if [ -f "/etc/alpine-release" ] || [[ "$SYSTEM_NAME" == *"BSD" ]] || [[ "$OS_MODE" == "android" ]] || [[ "$OS_MODE" == "windows" && "$SYSTEM_NAME" == *"-ARM64" ]]; then
...
run_staticlib_tests() {
    ...
    if [[ "$OS_MODE" == "windows" ]]; then
        ...
        if [[ "$SYSTEM_NAME" == *"OpenBSD"* ]]; then
             ...
             cc "$ROOT_DIR/resources/examples/staticlib-test/test.c" -L. -ladd -lexecinfo -lm -lelf -pthread -o a.out
        else
